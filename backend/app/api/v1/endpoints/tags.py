...
async def create_tag(
    ...
    tag = await service.create_tag(current_user_id, data)
    return tag
...
async def list_tags(
    ...
    service = TagService(db)
    tags = await service.list_tags(current_user_id, search, include_system)
...
async def get_popular_tags(
    ...
    service = TagService(db)
    tags = await service.get_popular_tags(current_user_id, limit)
    return TagListResponse(tags=[TagListItem.from_orm(tag) for tag in tags], total=len(tags))
...
async def autocomplete_tags(
    ...
    service = TagService(db)
    tags = await service.autocomplete_tags(current_user_id, prefix, limit)
...
async def get_tag(
    ...
    service = TagService(db)
    tag = await service.get_tag(tag_id, current_user_id)
    return tag
...
async def update_tag(
    ...
    """Update a tag."""
    service = TagService(db)
    tag = await service.update_tag(tag_id, current_user_id, data)
    return tag
...
async def delete_tag(
    ...
    service = TagService(db)
    await service.delete_tag(tag_id, current_user_id)
    return None
...
async def merge_tags(
    ...
    service = TagService(db)
    tag = await service.merge_tags(data.source_tag_id, data.target_tag_id, current_user_id)
    return tag