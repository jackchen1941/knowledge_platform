...
def include_object(object, name, type_, reflected, compare_to):
    """Include object in migration based on custom rules."""
    # Skip temporary tables
    if type_ == "table" and name.startswith("temp_"):
        return False
    
    # Skip certain indexes that are auto-generated
    if type_ == "index" and name.startswith("sqlite_autoindex"):
        return False
...
async def run_async_migrations() -> None:
    ...
    from app.core.database import get_database_type, get_engine_config
    
    db_type = get_database_type(settings.DATABASE_URL)
    engine_config = get_engine_config(db_type)
    
    # Override some settings for migrations
    engine_config.update({
        "poolclass": pool.NullPool,  # Use NullPool for migrations
        "echo": False,  # Reduce noise during migrations
    })
